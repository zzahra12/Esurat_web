// ==========================================================================
// 1. GENERATE PREVIEW SURAT PINDAH ALAMAT (PDA)
// ==========================================================================
function generatePreview(event) {
    ...
    try {
        ...
        // Data Detail PDA (Pindah Alamat)
        const alamatLama = getValue('alamatLama', '-');
        const alamatBaru = getValue('alamatBaru', '-');
        const noTelpLama = getValue('noTelpLama', '-');
        const noTelpBaru = getValue('noTelpBaru', '-');
        const noInternetLama = getValue('noInternetLama', '-');
        const noInternetBaru = getValue('noInternetBaru', '-');
        const keterangan = getValue('keteranganTambahan', '-');

        const namaTelkom = getValue('namaTelkom', 'Yustika Monita');
        const isTtdChecked = document.getElementById('checkTtdMonita')?.checked || false;
        ...
        setText('prevAlamatLayanan', alamatLama);

        // Populate Detail Poin PDA
        setText('prevAlamatLama', alamatLama);
        setText('prevAlamatBaru', alamatBaru);
        setText('prevNoTelpLama', noTelpLama);
        setText('prevNoTelpBaru', noTelpBaru);
        setText('prevNoInternetLama', noInternetLama);
        setText('prevNoInternetBaru', noInternetBaru);
        setText('prevKeterangan', keterangan);
        ...
        console.error("Gagal memproses preview Surat PDA:", error);
        alert("Terjadi kesalahan saat membuat preview.");
    }
}

// ==========================================================================
// 2. DOWNLOAD PDF (SURAT PDA - MARGIN ATAS 18MM & PRESISI A4)
// ==========================================================================
async function downloadPDF() {
    ...
    // --- STYLE INJECTION: MARGIN ATAS 18MM & SPASI KONSISTEN PDA ---
