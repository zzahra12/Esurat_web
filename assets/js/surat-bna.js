...
function generatePreview(event) {
    ...
    try {
        ...
        // Detail Surat Balik Nama (BNA) Khas BNA
        const noAkunLama = getValue('noAkunLama', '-');
        const noAkunBaru = getValue('noAkunBaru', '-');
        ...
        setText('prevNoAkunLama', noAkunLama);
        setText('prevNoAkunBaru', noAkunBaru);
...
async function downloadPDF() {
    ...
    // --- STYLE INJECTION: MARGIN ATAS 18MM & SPASI KONSISTEN BNA ---
    ...
    try {
        ...
        await doc.html(element, {
            callback: function (pdf) {
                pdf.save(`Surat_BNA_${namaPelanggan.replace(/\s+/g, '_')}.pdf`);
            },
            x: 0,
            y: 0,
            width: 210,
